function [methyl_param] = glm_param(index_list,trj)
[m,n]=size(trj);
x=10;
tao=x*500;
...
if index_list(1)==0
    ...
    %%%%%%% C6-C9 of DHF %%%%%%%%%%%%%%%%%%%%%%%
    c_a=trj(:,index_list(1):index_list(1)+2); % C5
    c_a=transpose(c_a);
    c_b=trj(:,index_list(2):index_list(2)+2); % C7
    c_b=transpose(c_b);
    ...
    save('TMPP_Corr_Func_C5_C7.dat','s_corr','-ascii')
    ...
    xlabel('Time (ns)')
    ylabel('C(t)')
    savefig(h,sprintf('TMPP_Corr_Func_C5_C7.fig'))
    close(h)
    ...
    c_9=trj(:,index_list(2):index_list(2)+2); % C7
    c_9=transpose(c_9);
    n_10=trj(:,index_list(3):index_list(3)+2); % C1P
    n_10=transpose(n_10);
    ...
    %saving second order correlation function data
    save('TMPP_Corr_Func_C7_C1P.dat','s_corr', '-ascii')
    x_corr=linspace(0,10,tao);
    h=figure;
    plot(x_corr,s_corr)
    ylim([0 1])
    ...
    xlabel('Time (ns)')
    ylabel('C(t)')
    savefig(h,sprintf('TMPP_Corr_Func_C7_C1P.fig'))
    close(h)
    %{
    %%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%% N3-C4 of DHF %%%%%%%%%%%%%%%%%%%%%%%%%
